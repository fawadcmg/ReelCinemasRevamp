...
var initHashValue = window.location.hash;
function clickDate() {
	if(initHashValue){
		var targetItem1 = $('[attr-movie-date="'+initHashValue.split('#')[1]+'"]');
		if(targetItem1.get(0)){
	...
	$('.js-date-time').slick('slickGoTo', parseInt($('.dboxelement.active').closest('.slick-slide').attr('data-slick-index')) - $('.js-date-time .slick-active').length + 1);
}
...
function loadMoviesDropdown(){
	...
	$.getJSON('MoviesSession.json', function (data) {
		...
		for (arrayIndex = 0; arrayIndex < tempArray.length; arrayIndex++) {
			tempEntry = tempArray[arrayIndex];				
			moviesListing.find('.scroll-area').append('<div class="item"><input type="checkbox" value="movie-'+tempEntry[1]+'" class="js-movieItem" id="'+tempEntry[1]+'"><label for="'+tempEntry[1]+'">'+tempEntry[0]+'</label></div>');
		}
...
function loadMoviesComingDropdown(){	
	...
	$.getJSON('ComingSoon.json', function (data) {
		...
		for (arrayIndex = 0; arrayIndex < comingTempArray.length; arrayIndex++) {
			tempEntry = comingTempArray[arrayIndex];				
			comingMoviesListing.find('.scroll-area').append('<div class="item"><input type="checkbox" value="movie-'+tempEntry[1]+'" class="js-movieItem-coming" id="'+tempEntry[1]+"-coming"+'"><label for="'+tempEntry[1]+"-coming"+'">'+tempEntry[0]+'</label></div>');
		}
		comingMoviesListing.append('<div class="item item--close"><a href="javascript:void(0);" class="js-close-custom-select">Close</a></div>');
...
function initMovieGrid(){
	...
	$.getJSON('MoviesSession.json', function (data) {
		$.each( data, function( i, item ) {
			...
			movieLanguage = movieLanguage.replace(/ Language: /g, "");
			movieLanguage = movieLanguage.replace(/ Language/g, "");
			movieLanguage = movieLanguage.replace(/Subtitle: /g, "</div><div class='info language'><i class='icon medium'><picture><source srcset='assets/img/icons/subtitles.svg' media='(max-width: 767px)''><img src='assets/img/icons/subtitles--white.svg' alt=''></picture></i><span>");
			movieLanguage = movieLanguage.replace(/Subtitle /g, "</div><div class='info language'><i class='icon medium'><picture><source srcset='assets/img/icons/subtitles.svg' media='(max-width: 767px)''><img src='assets/img/icons/subtitles--white.svg' alt=''></picture></i><span>");
			...
			tempMovieSynopsis = movieSynopsis.split('\n');
		 	...
		 	for (arrayIndex = 0; arrayIndex < movieExperienceArray.length; arrayIndex++) {
				...
				if(movieExperienceTemp.indexOf(temp[0]) > -1  ){					
					movieExperience += '<li><picture><source srcset="assets/img/logos/logo-reel-'+temp[1]+'--white.png" media="(max-width: 767px)"><img src="assets/img/logos/logo-reel-'+temp[1]+'--white.png" alt="Reel '+temp[0]+'"></picture></li>';
				}				
			}
...
function loadMovieGridBlocks(argMovie){
		...
		for(var innerCounter=0; innerCounter < initMovieExperienceCinema.length; innerCounter++){			
			...
			if(initMovieExperienceCinema[innerCounter].indexOf(tempCinameExperience) > -1 ){
				for(genreCount=0; genreCount < tempMovieGenreArray.length; genreCount++){
					movieCinemaExperienceClass += "movie-"+initMovieExperienceCinema[innerCounter]+"-genre-"+tempMovieGenreArray[genreCount].trim()+" ";
				}				
			}
		}
	...
	var genereArray = tempEntry[11].split(' | ');
	var genreDiv = '<div class="info genere">';

	for (var i =  0; i < genereArray.length; i++) {
		genreDiv += '<span>'+genereArray[i]+'</span>';
	}
	...
	<section class="item-details"> <a href="javascript:void(0);" class="btn-close js-close-movie-list-detail"><i></i><i></i><span class="txt">close</span></a>\
		<div class="text"> <div class="title-wrap"> <h1 class="title">'+tempEntry[4]+'</h1> <div class="stamp">'+tempEntry[9]+'</div></div>'+
			genreDiv
			+'<div class="info duration"><i class="icon"><picture><source srcset="assets/img/icons/duration.svg" media="(max-width: 767px)"><img src="assets/img/icons/duration--white.svg" alt="FB" class="svg"></picture></i><span>'+tempEntry[8]+'</span></div>\
...
function initComingMovieGrid(){
	...
	$.getJSON('ComingSoon.json', function (data) {
		$.each( data, function( i, item ) {
			...
			movieLanguage = movieLanguage.replace(/ Language: /g, "");
			movieLanguage = movieLanguage.replace(/ Language/g, "");
			movieLanguage = movieLanguage.replace(/Subtitle: /g, "</div><div class='info language'><i class='icon medium'><img src='assets/img/icons/subtitles--white.svg' alt='' class=''></i><span>");
			movieLanguage = movieLanguage.replace(/Subtitle /g, "</div><div class='info language'><i class='icon medium'><img src='assets/img/icons/subtitles--white.svg' alt='' class=''></i><span>");
			...
			movieCinemaClass = movieCinemaClass.replace(/,/g, " ");*/
			movieCinemaClass = movieCinemaClass.toLowerCase();
			var tempCinemaClass = movieCinemaClass.split(",");
			...
			for(var cinemaCount=0; cinemaCount < tempCinemaClass.length; cinemaCount++){
				tempCinemaClass[cinemaCount] = findAndReplace(tempCinemaClass[cinemaCount]," ","-");
				for(var counter=0; counter < movieGenreClassArray.length; counter++){
					movieItemClass += "movie-"+movieNameClass + "-" + tempCinemaClass[cinemaCount].toLowerCase() + "-genre-" +movieGenreClassArray[counter].trim().toLowerCase()+" ";
			 	}
			}
...
function loadComingMovieGridBlocks(){
	...
	var genereArray = tempEntry[10].split(' | ');
	var genreDiv = '<div class="info genere">';
...
function initMovieDates(argMovieName){
	...
	$.getJSON('Sessions.json', function (data) {
		$.each( data, function( i, item ) {
			...
			showMovie = 0;			
			movieDate = new Date (item.SessionDate);
			movieName = item.MovieName;
			if(argMovieName == movieName || argMovieName == "all"){
				showMovie = 1;
			}
			
			var movieDateValue = movieDate;
			movieDate = movieDateValue.getDate()+"/"+(movieDateValue.getMonth()+1)+"/"+movieDateValue.getFullYear();
		...
		if(tempArray.length > 0){
			...
			targetItem.slick('init', function (slick) {
				clickDate();
			});
			...
			$('.js-movieDateFilter').click(function () {
				...
				window.location.hash = movieDateFilter;

				$('.c-movie-filters .viewbar a').each(function () {
					var thisHref = $(this).attr('href');
					if(!(thisHref == '#' || thisHref == 'javascript:void(0)')){
						thisHref = thisHref.split('#')[0] + '#' + movieDateFilter;
		...
		$.getJSON('ComingSoon.json', function (data) {
			$.each( data, function( i, item ) {
				...
				if(movieName == argMovieName){
					...
					movieGenre = item.Genre.split(',').join(' | ');
				 	...
				 	for(var counter=0; counter < tempMovieLanguage.length; counter++){
				 		if (tempMovieLanguage[counter].indexOf('Language') > -1) {
				 			strLen = tempMovieLanguage[counter].length;
				  			...
				  			movieLanguage = '<i class="icon medim"><img src="assets/img/icons/language--white.svg" alt="FB" class="svg"></i><span>'+tempLanguage+'</span>';
			            }            
			            if (tempMovieLanguage[counter].indexOf('Subtitle') > -1) {	                
			                strLen = tempMovieLanguage[counter].length;
				  			...
				  			movieSubtitle =  '<i class="icon medium"><picture><source srcset="assets/img/icons/subtitles--white.svg" media="(max-width: 767px)"><img src="assets/img/icons/subtitles--white.svg" alt="FB" class=""></picture></i><span>'+tempSubtile+'</span>'			  			
			            }
				 	}
		...
		$.getJSON('MoviesSession.json', function (data) {
			$.each( data, function( i, item ) {
				...
				if(movieName == argMovieName){
					...
					var movieGenreArry = item.Genre.split(',');
					movieGenre = '';
					for (var i = 0; i <= movieGenreArry.length; i++) {
						movieGenre += '<span>'+movieGenreArry[i]+'</span>';
					}
	...
	if(showTime){
		...
		if($('.showtime-tile-page').length > 0 ){
			movieLabel = '<div class="img">\
		                     ...
		                     <div class="moviename"><div class="txt-hover-wrap"><span>'+argMovieName+'</span><strong>'+argMovieName+'</strong></div></div>\
		...
		for(counter=0; counter < showtimeClass.length; counter++){
			itemClass += " movie-"+argMovieName+'-'+movieCinemaClass+'-'+movieExperienceClass+"-"+showtimeClass[counter]+"-"+showtimeClass[counter];
		}
		...
		result = '<div class="tileview-movies-list '+itemClass+' '+hideMovieClass+'">\
	                     <div class="item">\
	                        <div class="movielocation">\
	                           <div class="txt-hover-wrap"><span>'+argCinemaName+'</span><strong>'+argCinemaName+'</strong></div>\
	                        </div>\
	                     </div>\
	...
	if( argMoviesIDs.length == 0 
		...
		if(argGenreIDs.length > 0 ){	
			...
			for(counter=0; counter < argGenreIDs.length; counter++){				
				for(innerCounter=0; innerCounter < movieItems.length; innerCounter++){
					var findItem = "-genre-"+argGenreIDs[counter];					
					if( movieItems[innerCounter].indexOf(findItem) > -1 ){
						tempArray.push(movieItems[innerCounter]);
					}	
				}
...
$(".o-container p").text(function(index, currentText) {
