...
function loadCinemas(){
	...
	$.getJSON('Cinemas.json', function (data) {
		...
		$('.js-cinemaItem').click(function () {
			...
			filterMoviesListing(moviewFilter, cinemaFilter, experienceFilter, genreFilter, movieDateFilter);
		});
...
function loadMovieDates(){	
	...
	var movieDate, movieNameClass, movieName, itemClass;
		...
		$.each( data, function( i, item ) {  
			...
			for(counter=0; counter < MovieListingArray.length; counter++){
				if(MovieListingArray[counter].indexOf(movieNameClass) > -1){
					MovieListingArray[counter] = MovieListingArray[counter].replace("movie-item", "movie-item "+itemClass);
				}
			}
...
function filterMoviesListing(movieIDs, cinemaIDs, experienceIDs, genreIDs, movieDates){
	...
	alert(movieIDs + cinemaIDs + experienceIDs + genreIDs + movieDates);
