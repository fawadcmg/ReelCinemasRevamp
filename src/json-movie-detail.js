...
function loadCinamaListing(movieName){
  	...
  	for(counter=0; counter< movieCinamaListing.length; counter++){	
  		movieExprerience = "";				
  		showTime = "";
  		tempValue1 = movieCinamaListing[counter];
		...
		// if(movieCinamaListing[counter] == movieCinema){
			...
			$.getJSON('Sessions.json', function (data) {
				$.each( data, function( i, item ) {
					...
					// console.log(movieName1 +'-'+ tempMovieName +'-'+ tempValue +'-'+ item.CinemaName);
					// console.log(tempValue +'-'+ item.CinemaName);
					console.log(tempValue1 +'----'+ movieCinema);
				...
				console.log("Movies cinema listing completed for " + tempValue1);
			}).fail(function( data ) {
			    console.log("Movies cinema listing failed for " + tempValue1);
			});
		// }

		console.log(movieExprerience + showTime);
